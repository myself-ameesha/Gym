...
const MemberDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (currentMember?.id && !dataLoaded) {
      ...
      loadData();
    } else if (!currentMember?.id && !loading && (isAuthenticated || token)) {
      console.log('Getting current member...');
      dispatch(getCurrentMember()).catch((error) => {
  ...
  }, [dispatch, currentMember, dataLoaded, loading, activeSection, isAuthenticated, token, navigate]);
  ...
  const handlePayment = useCallback(async () => {
    ...
    try {
      ...
      // If there's an authentication error during payment, redirect to login
      if (err.response?.status === 401 || err.response?.status === 403) {
        localStorage.removeItem("accessToken");
        navigate('/login');
      }
    }
  }, [selectedPlanId, currentMember, dispatch, membershipPlans, navigate]);
  ...
  // Show loading if not authenticated or no token - but don't return early
  const showAuthLoading = !isAuthenticated && !token;

  // Show loading screen for unauthenticated users, but don't early return
  if (showAuthLoading) {
    return (
      <div className="text-center my-5" style={{ backgroundColor: '#0c1427', minHeight: '100vh' }}>
        <Spinner animation="border" variant="light" />
  ...
  // Show loading if not authenticated or no token
  if (!isAuthenticated && !token) {
    return (
      <div className="text-center my-5" style={{ backgroundColor: '#0c1427', minHeight: '100vh' }}>
        <Spinner animation="border" variant="light" />
