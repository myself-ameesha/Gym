...
const Register = () => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      ...
      setApiError({ type: "success", message: "Registration successful! Redirecting to payment..." });

      // Get selected membership plan details
      const selectedPlan = membershipPlans.find(plan => plan.id === Number(values.membership_plan_id));

      // Navigate to payment page with user data and selected plan
      setTimeout(() => {
        navigate('/payment', { 
          state: { 
            userData: {
              ...
              fitness_goal: values.fitness_goal,
              userId: response.data.user_id || response.data.id // Adjust based on your API response structure
            },
          ...
          {({ values }) => (
            <Form>
              ...
              <button
                ...
                disabled={isSubmitting || loading}
              >
                {isSubmitting ? "Registering..." : "Register & Continue to Payment"}
              </button>
            </Form>
          )}
        </Formik>
