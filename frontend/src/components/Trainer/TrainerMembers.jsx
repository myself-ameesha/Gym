...
const TrainerMembers = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch assigned members first
        const membersResult = await dispatch(getAssignedMembers()).unwrap();
        console.log('Assigned Members fetched:', membersResult);

        // Fetch attendance history for each member
        if (membersResult && membersResult.length > 0) {
          const fetchPromises = membersResult.map(async (member) => {
            try {
              const result = await dispatch(getAttendanceHistory(member.id)).unwrap();
              console.log(`Attendance history for member ${member.id}:`, result);
              return result;
            } catch (err) {
              console.error(`Error fetching attendance history for member ${member.id}:`, err);
              // Don't throw error for individual member fetch failures
              return null;
            }
          });
        ...
        console.error('Error fetching data on mount:', err);
        setLocalAttendanceError('Failed to fetch members');
      }
    };
  ...
  const handleSubmitAttendance = async () => {
    if (!currentMember) {
      setLocalAttendanceError('No member selected');
      return;
    }

    try {
      // Check for existing record
      const memberRecords = attendanceRecords[currentMember.id] || [];
      const existingRecord = memberRecords.find(
        (record) => record.date === attendanceDate
      );
      ...
      // Refresh attendance history for the member
      try {
        await dispatch(getAttendanceHistory(currentMember.id)).unwrap();
      } catch (refreshErr) {
        console.error('Error refreshing attendance history:', refreshErr);
        // Don't show error for refresh failure as the attendance was marked successfully
      }

    } catch (err) {
      console.error('Error marking attendance:', err);
      const errorMessage = err || 'Failed to mark attendance';
  ...
  const handleViewHistory = async (member) => {
    ...
    try {
      // Fetch latest attendance history
      await dispatch(getAttendanceHistory(member.id)).unwrap();
      setShowHistoryModal(true);
    } catch (err) {
      console.error('Error fetching attendance history:', err);
      setLocalAttendanceError('Failed to fetch attendance history');
  ...
  const getCurrentMemberRecords = () => {
    if (!currentMember || !attendanceRecords[currentMember.id]) {
      return [];
    }
    return [...attendanceRecords[currentMember.id]]; // Return a copy
  };
    ...
    <div className="flex-grow-1 p-4" style={{ backgroundColor: '#0c1427', minHeight: '100vh' }}>
      ...
      <Modal show={showHistoryModal} onHide={() => setShowHistoryModal(false)} size="lg">
        ...
        <Modal.Body>
          ...
          ) : getCurrentMemberRecords().length === 0 ? (
