...
const TrainerDashboard = () => {
  ...
  // Updated getImageUrl function for Cloudinary
  const getImageUrl = (imageUrl) => {
    ...
    if (imageUrl.includes('cloudinary') || !imageUrl.startsWith('/')) {
      return imageUrl;
    }
      ...
      dispatch(getCurrentTrainer())
        ...
        .then(data => {
          ...
          if (data && typeof data === 'object' && !Array.isArray(data)) {
            ...
            // Set image preview from Cloudinary URL
            const imageUrl = getImageUrl(data.trainer_profile?.profile_img);
            if (imageUrl) {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      // Update image preview with new Cloudinary URL
      const newImageUrl = getImageUrl(updatedTrainer.trainer_profile?.profile_img);
      if (newImageUrl) {
