...
const DietPlanManagement = () => {
  ...
  const handleEditDietPlan = (member, plan) => {
    console.log('Editing diet plan:', { member, plan }); // Debug log
    setCurrentMember(member);
    setSelectedMember(member.id);
    setDietForm({
      default_diet_plan_id: plan.default_diet_plan ? plan.default_diet_plan.id.toString() : '',
  ...
  const handleDeleteDietPlan = (planId) => {
    console.log('Attempting to delete diet plan:', { planId, currentMember }); // Debug log
    
    let planExists = false;
    let plan = null;
    
    if (currentMember && dietPlans[currentMember.id]) {
      plan = dietPlans[currentMember.id].find(p => p.id === planId);
      planExists = !!plan;
    }
    
    console.log('Plan exists check:', { planExists, plan }); // Debug log
  ...
  const confirmDeleteDietPlan = () => {
    console.log('Confirming delete for plan:', deletingDietPlanId); // Debug log
    ...
    dispatch(deleteDietPlan(deletingDietPlanId)).then((result) => {
      ...
      if (result.meta.requestStatus === 'fulfilled') {
        ...
        // Refresh the diet plan history
        if (currentMember) {
          dispatch(getDietPlanHistory(currentMember.id));
        }
      } else {
        // Handle the error case
        const errorMessage = result.payload?.error || 'Failed to delete diet plan';
        toast.error(errorMessage);
        console.error('Delete failed:', result.payload);
      }
    }).catch((error) => {
      console.error('Delete error:', error);
      toast.error('An unexpected error occurred while deleting the diet plan');
    });
  };
  ...
  const handleViewCurrentDiet = (member) => {
    ...
    dispatch(getCurrentDietPlan(member.id)).then((result) => {
      console.log('Current diet result:', result); // Debug log
      
      if (result.meta.requestStatus === 'fulfilled') {
        setShowCurrentDietModal(true);
      } else if (result.payload?.status === 404) {
        // No active diet plan found - this is not an error
        setShowCurrentDietModal(true);
      } else {
        // Actual error
        const errorMessage = result.payload?.error || 'Failed to fetch current diet plan';
        toast.error(errorMessage);
      }
    });
  };
  ...
  const handleSubmitDiet = () => {
    ...
    console.log('Editing Diet Plan:', editingDietPlan);
    console.log('Is Editing?', !!editingDietPlan);
    console.log('Diet Plan Data being sent:', dietPlanData);
    
    if (editingDietPlan) {
      console.log('Original diet plan details:', {
        id: editingDietPlan.id,
        title: editingDietPlan.title,
        trainer: editingDietPlan.trainer,
        trainer_email: editingDietPlan.trainer_email,
        member: editingDietPlan.member,
        member_email: editingDietPlan.member_email
      });
    }
    ...
    dispatch(action).then((result) => {
      ...
      if (result.meta.requestStatus === 'fulfilled') {
        ...
        // Handle the error case
        const errorMessage = result.payload?.error || 'Operation failed';
        toast.error(errorMessage);
              ...
              <tbody>
                {dietPlans[currentMember?.id].map((plan) => (
                  <tr key={plan.id}>
                    ...
                    <td>{formatDate(plan.created_at)}</td>
                    <td>
  <div className="d-flex gap-2"> {/* Added gap-2 class for spacing */}
    <Button
      variant="outline-warning"
      size="sm"
      onClick={() => handleEditDietPlan(currentMember, plan)}
