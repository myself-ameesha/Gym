...
class PaidMembersListView(APIView):
    ...
    def get(self, request):
        try:
            ...
            paid_members = User.objects.filter(
                user_type="member", has_paid=True, is_active=True
            )
            logger.info(
                f"Fetching paid members for admin: {request.user.email}, found {paid_members.count()} members"
            )
...
class CreateTrainerView(APIView):
    ...
    def post(self, request):
        logger.info(
            f"Trainer creation attempt by {request.user.email} at {datetime.now()}"
        )
...
class CreateMembershipPlanView(APIView):
    ...
    def post(self, request):
        try:
            logger.info(
                f"Membership plan creation attempt by {request.user.email} at {datetime.now()}"
            )
            ...
            if MembershipPlan.objects.filter(name=name).exists():
                logger.warning(
                    f"Attempt to create duplicate membership plan with name: {name}"
                )
                return Response(
            ...
            logger.info(
                f"Membership plan {name} created successfully by {request.user.email}"
            )
            return Response(response_data, status=status.HTTP_201_CREATED)

        except Exception as e:
            logger.error(
                f"Error creating membership plan: {str(e)}\nRequest data: {request.data}"
            )
            return Response(
...
def edit_membership_plan(request, pk):
    ...
    try:
        ...
        return Response(
            {"error": "Membership plan not found"}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    try:
        ...
        if trainer_id == 0 or trainer_id is None:
            ...
            try:
                ...
                return Response(
                    {"error": "Invalid trainer ID"}, status=status.HTTP_400_BAD_REQUEST
                )
...
def get_trainer_members(request, trainer_id):
    ...
    try:
        ...
        members = User.objects.filter(
            user_type="member", assigned_trainer=trainer
...
class RevenueView(APIView):
    ...
    def get(self, request):
        try:
            ...
            logger.info(
                f"Revenue request from {request.user.email} with dates: start={start_date}, end={end_date}"
            )
            ...
            try:
                ...
                return Response(
                    {
                        "error": "Database error while filtering payments",
                        "details": str(e),
                    },
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                )
            ...
            try:
                ...
                for payment in payments.select_related("user", "membership_plan"):
                    try:
                        ...
                        payment_detail = {
                            "id": payment.id,
                            "user_email": (
                                getattr(payment.user, "email", "N/A")
                                if payment.user
                                else "N/A"
                            ),
                ...
                logger.info(
                    f"Successfully processed {len(payment_details)} payment details"
                )
            ...
            try:
                ...
                logger.info(
                    f"Revenue data fetched successfully by {request.user.email}: Total={total_revenue}, Count={len(payment_details)}"
                )
                return Response(response_data, status=status.HTTP_200_OK)
...
class SalesReportView(APIView):
    ...
    def get(self, request):
        try:
            ...
            logger.info(
                f"Sales report request from {request.user.email} with dates: start={start_date}, end={end_date}"
            )
            ...
            payment_details = [
                {
                    "id": payment.id,
                    "user_email": (
                        getattr(payment.user, "email", "N/A") if payment.user else "N/A"
                    ),
                    "user_name": f"{getattr(payment.user, 'first_name', '')} {getattr(payment.user, 'last_name', '')}".strip()
                    or "N/A",
                    "membership_plan": (
                        getattr(payment.membership_plan, "name", "N/A")
                        if payment.membership_plan
                        else "N/A"
                    ),
                    "amount": (
                        float(payment.amount) if payment.amount is not None else 0.0
                    ),
                    "created_at": (
            ...
            logger.info(
                f"Sales data fetched: Total={total_revenue}, Count={len(payment_details)}"
            )
            return Response(response_data, status=status.HTTP_200_OK)

        except Exception as e:
            logger.error(
                f"Unexpected error in SalesReportView: {str(e)}", exc_info=True
            )
            return Response(
                {"error": "An unexpected error occurred", "details": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    return Response(
                        {
                            ...
                            "details": "Please install texlive-full",
                        },
                        status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    )
                ...
                if os.path.exists(pdf_file_path):
                    ...
                    response["Content-Disposition"] = (
                        f'attachment; filename="sales_report_{start_date or "all"}_{end_date or "all"}.pdf"'
                    )
                    ...
                    return Response(
                        {
                            ...
                            "details": "No PDF file was created",
                        },
                        status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    )
...
class MarkTrainerAttendanceView(APIView):
    ...
    def post(self, request):
        try:
            ...
            serializer = TrainerAttendanceSerializer(
            ...
            if serializer.is_valid():
                ...
                attendance = TrainerAttendance(
                    trainer=trainer, admin=request.user, date=date, status=status_val
                )
                attendance.save()
        ...
        try:
            ...
            for trainer in trainers:
                ...
                try:
                    ...
                    trainer_data = {**user_serializer.data, "trainer_profile": None}
